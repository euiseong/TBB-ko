...
\begin{tbbitemize}
  ...
  \item \textbf{② 하나를 손으로 스트리밍한다.} 기록 11-5처럼 \code{"stream": true}와 \code{curl -N}을 사용하고 SSE 청크가 도착하는 모습을 \textit{지켜본다}. 잠깐 숨을 고른 뒤 첫 청크(TTFT)가 오고 이어 일정한 가랑비(TPOT)가 내린다. 어떤 하네스(harness)가 추상화하기 전에 두 시계를 직접 눈으로 본다.
