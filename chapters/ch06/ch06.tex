...
클라우드 스토리지는 세 계열로 나뉜다. 이들을 생산적으로 비교하는 방법은 기능 목록이 아니라 서비스가 무엇을 약속하고 그 약속에 얼마를 받는지 나타내는 \textbf{계약}을 보는 것이다. 계약을 한 번 정확히 이해하면 이 강의의 모든 스토리지 결정은 패턴 맞추기로 단순해진다. 접근 패턴은 무엇이고, 누가 공유하며, 어느 정도의 내구성(durability)이 필요하고, 정확히 그 조건을 파는 계약은 무엇인가(그림 6-2)?
